...
public record MazeGeneratorCommand(MazeGeneratorPlugin plugin) implements CommandExecutor, Listener {
    ...
    private static final String[] algorithms =
            {"A* 2D", "A* Diagonal 2D", "A* 3D", "A* Diagonal 3D", "Greedy BFS 2D", "GBFS Diagonal 2D",
                    "Greedy BFS 3D", "GBFS Diagonal 3D", "Breadth-FS 2D", "Breadth-FS 3D", "Depth-FS 2D",
                    "Depth-FS 3D"};
    ...
    public void onButtonPressed(PlayerInteractEvent e) {
            ...
            if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
                if (Objects.equals(e.getClickedBlock(), controlPlatform.getStartButton())) {
                    ...
                    if (blockerPercentage < .80) {
                        ...
                        blockerPercentage = (double) Math.round(blockerPercentage * 100) / 100;
                ...
                switch (index) {
                    ...
                    case 1 -> maze = new AstarMazeDiagonal2D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
                    );
                    case 2 -> maze = new AstarMaze3D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
                    ...
                    case 5 -> maze = new GreedyBestFirstSearchMazeDiagonal2D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
                    );
                    case 6 -> maze = new GreedyBestFirstSearchMaze3D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
                    );
                    case 7 -> maze = new GreedyBestFirstSearchMazeDiagonal3D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
                    );
                    case 8 -> maze = new BreadthFirstSearchMaze2D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
                    );
                    case 9 -> maze = new BreadthFirstSearchMaze3D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
                    );
                    case 10 -> maze = new DepthFirstSearchMaze2D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
                    );
                    case 11 -> maze = new DepthFirstSearchMaze3D(
                            plugin,
                            mazeLocation.getBlock(),
                            size,
                            blockerPercentage
        ...
        System.out.println("PERCENTAGE - " + blockerPercentage);
    }
}
